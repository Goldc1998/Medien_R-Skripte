...
bigram_data <- text_data %>%
  ...
  rename(bigram = pair, frequency = n)  # Benenne Spalten für Klarheit
...
bigram_plot <- ggplot(bigram_data[1:30, ], aes(x = reorder(bigram, frequency), y = frequency)) +
  ...
  coord_flip() +
  labs(title = "30 häufigste Bigramme mit 'Israel'",
       x = "Bigramme",
       y = "Häufigkeit") +
  theme_minimal()
...
trigram_data <- text_data %>%
  ...
  rename(trigram = triplet, frequency = n)  # Benenne Spalten für Klarheit
